...
def _scope_filters(franchise_id, warehouse_id) -> list:
    ...
    if franchise_id:
        return [ParcelOrder.franchise_id == franchise_id]
    if warehouse_id:
        return [ParcelOrder.warehouse_id == warehouse_id]
...
async def create_parcel_order(
    ...
    - Sender and receiver details are stored flat (no separate table).
    ...
    parcel = ParcelOrder(
        ...
        # Sender
        sender_name=data.sender.name if data.sender else None,
        sender_mobile=data.sender.mobile if data.sender else None,
        sender_alternate_mobile=data.sender.alternate_mobile if data.sender else None,
        sender_email=data.sender.email if data.sender else None,
        sender_address_line_1=data.sender.address_line_1 if data.sender else None,
        sender_address_line_2=data.sender.address_line_2 if data.sender else None,
        sender_pincode=data.sender.pincode if data.sender else None,
        sender_city=data.sender.city if data.sender else None,
        sender_state=data.sender.state if data.sender else None,
        

        # Receiver
        receiver_name=data.receiver.name if data.receiver else None,
        receiver_mobile=data.receiver.mobile if data.receiver else None,
        receiver_alternate_mobile=data.receiver.alternate_mobile if data.receiver else None,
        receiver_email=data.receiver.email if data.receiver else None,
        receiver_address_line_1=data.receiver.address_line_1 if data.receiver else None,
        receiver_address_line_2=data.receiver.address_line_2 if data.receiver else None,
        receiver_pincode=data.receiver.pincode if data.receiver else None,
        receiver_city=data.receiver.city if data.receiver else None,
        receiver_state=data.receiver.state if data.receiver else None,
    ...
    if search:
        ...
        filters.append(
            or_(
                ...
                ParcelOrder.sender_name.ilike(like),
                ParcelOrder.sender_mobile.ilike(like),
                ParcelOrder.receiver_name.ilike(like),
                ParcelOrder.receiver_mobile.ilike(like),
                ParcelOrder.receiver_pincode.ilike(like),
                ParcelOrder.sender_pincode.ilike(like),
            )
        )

    where = and_(*filters) if filters else True

    total = (await db.execute(select(func.count()).select_from(ParcelOrder).where(where))).scalar_one()
...
async def update_parcel_order(
    ...
    # Flatten sender / receiver nested objects into flat columns
    if "sender" in update_data:
        sender = update_data.pop("sender") or {}
        mapping = {
            "sender_name": "name",
            "sender_mobile": "mobile",
            "sender_alternate_mobile": "alternate_mobile",
            "sender_email": "email",
            "sender_address_line_1": "address_line_1",
            "sender_address_line_2": "address_line_2",
        ...
        for col, key in mapping.items():
            if key in sender:
                update_data[col] = sender[key]

    if "receiver" in update_data:
        receiver = update_data.pop("receiver") or {}
        mapping = {
            "receiver_name": "name",
            "receiver_mobile": "mobile",
            "receiver_alternate_mobile": "alternate_mobile",
            "receiver_email": "email",
            "receiver_address_line_1": "address_line_1",
            "receiver_address_line_2": "address_line_2",
